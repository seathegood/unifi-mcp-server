...
async def block_client(
    ...
    if dry_run:
        logger.info(sanitize_log_message(f"DRY RUN: Would block client '{client_mac}' in site '{site_id}'"))
        log_audit(
...
async def unblock_client(
    ...
    if dry_run:
        logger.info(sanitize_log_message(f"DRY RUN: Would unblock client '{client_mac}' in site '{site_id}'"))
        log_audit(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            logger.info(sanitize_log_message(f"Unblocked client '{client_mac}' in site '{site_id}'"))
            log_audit(
                operation="unblock_client",
                parameters=parameters,
                result="success",
...
async def reconnect_client(
    ...
    if dry_run:
        logger.info(sanitize_log_message(f"DRY RUN: Would force reconnect for client '{client_mac}' in site '{site_id}'"))
        log_audit(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            logger.info(sanitize_log_message(f"Forced reconnect for client '{client_mac}' in site '{site_id}'"))
            log_audit(
                operation="reconnect_client",
                parameters=parameters,
                result="success",
...
async def authorize_guest(
    ...
    if dry_run:
        logger.info(
            sanitize_log_message(f"DRY RUN: Would authorize guest client '{client_mac}' for {duration}s in site '{site_id}'")
        )
        log_audit(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            logger.info(
                sanitize_log_message(f"Authorized guest client '{client_mac}' for {duration}s in site '{site_id}'")
            )
            log_audit(
                operation="authorize_guest",
                parameters=parameters,
...
async def limit_bandwidth(
    ...
    if dry_run:
        logger.info(
            sanitize_log_message(f"DRY RUN: Would apply bandwidth limits to client '{client_mac}' in site '{site_id}'")
        )
        log_audit(
    ...
    try:
        async with UniFiClient(settings) as client:
            ...
            logger.info(sanitize_log_message(f"Applied bandwidth limits to client '{client_mac}' in site '{site_id}'"))
            log_audit(
                operation="limit_bandwidth",
                parameters=parameters,
                result="success",
        ...
        logger.error(sanitize_log_message(f"Failed to apply bandwidth limits to client '{client_mac}': {e}"))
        log_audit(
            operation="limit_bandwidth",
            parameters=parameters,
            result="failed",
